...
/**
 * Helper types for disposer implementation
 */
type PromiseInvokeType = "catch" | "then" | "finally"

/**
 * Argument type for invocation
 */
type PromiseInvokeArg<Type extends PromiseInvokeType, T> = Type extends "then"
  ? T
  : Type extends "finally"
  ? void
  : Error

/**
 * Disposer type, runs on then/catch first-come first serve
 * - OR - on finally in sync mode with a catch wrapper
 */
export type FutureDisposerMode = "then-catch" | "finally"

/**
 * Disposer for promise
 */
export type FutureDisposer<T extends ToString> = (
  err?: Error,
  value?: T,
  future?: Future<T>
) => Promise<T> | T
...
export class Future<T extends ToString> {
  ...
  disposer(dispose: FutureDisposer<T>, mode: FutureDisposerMode = "finally") {
    ...
    const wrappedFn = <Type extends PromiseInvokeType>(
      type: PromiseInvokeType
    ) => (value?: PromiseInvokeArg<Type, T>) => {
      if (invoked) {
        if (type === "then") {
          return value
        } else if (type === "catch") {
          return Promise.reject(value as Error)
        } else {
          // FINALLY DISPOSER
        }
      } else {
        ...
        if (type === "then") {
          ...
          return Promise.resolve(dispose(null, value as T, this))
            .then(resolve)
            .catch(resolve)
          ...
          try {
            const res = dispose(null, value as T, this)
            if (isPromise(res)) {
              res.catch(onErr)
            }
          } catch (err) {
          ...
          const reject = () => Promise.reject(value)
          return Promise.resolve(dispose(value as Error, null, this))
            .then(reject)
            .catch(reject)
        }
      }
   ...
   */
  static ofPromiseCtor<T>(
    executor: (resolve: (x: T) => void, reject: (x: any) => void) => void
  ): Future<T> {
   ...
   */
  static ofCallback<T>(
    fn: (cb: (err: any, val: T) => void) => void
  ): Future<T> {
    return Future.ofPromiseCtor((resolve, reject) =>
      fn((err, data) => {
        if (err) {
          reject(err)
        } else {
  ...
  then<TResult1 = T, TResult2 = never>(
    ...
    return this.promise.then(
      ([x]) => onfulfilled(x),
      (rejected) =>
        onrejected ? onrejected(rejected) : Promise.reject<TResult2>(rejected)
    )
  }
  ...
  static traverse<T, U>(
    ...
    if (!opts) {
      return Future.of(
        Promise.all(Vector.ofIterable(elts).map((x) => fn(x).toPromise())).then(
          Vector.ofIterable
        )
      )
    }
  ...
  static find<T>(
    ...
    return success.flatMap(([originalFuture, option]) => {
      if (option.isSome() && p(option.get())) {
        ...
        return Future.find(
          origElts.removeFirst((future) => future === originalFuture),
          p
        )
      }
    })
  ...
  static liftAp<A, B>(
    fn: (x: A) => B
  ): (x: { [K in keyof A]: Future<A[K]> }) => Future<B> {
    return (x) => {
   ...
   */
  static liftA2<R1, R2, V>(
    fn: (v1: R1, v2: R2) => V
  ): (p1: Future<R1>, p2: Future<R2>) => Future<V> {
  ...
  filter(p: (x: T) => boolean, ifFail: (x: T) => any): Future<T> {
    return this.flatMap((x) => (p(x) ? Future.ok(x) : Future.failed(ifFail(x))))
  }
