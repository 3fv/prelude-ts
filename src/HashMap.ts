...
export class HashMap<K extends ToString, V> implements IMap<K, V> {
   ...
   */
  static ofIterable<K, V>(
    entries: Iterable<[K & WithEquality, V]>
  ): HashMap<K, V> {
    ...
    let startH = new EmptyHashMap<K, V>().put(
      curItem.value[0],
      curItem.value[1]
    ).hamt
   ...
   */
  map<K2, V2>(
    fn: (k: K & WithEquality, v: V) => [K2 & WithEquality, V2]
  ): HashMap<K2, V2> {
   ...
   */
  flatMap<K2, V2>(
    fn: (k: K, v: V) => Iterable<[K2 & WithEquality, V2]>
  ): HashMap<K2, V2> {
    return this.foldLeft(HashMap.empty<K2, V2>(), (soFar, cur) =>
      soFar.mergeWith(fn(cur[0], cur[1]), (a, b) => b)
    )
  }
  ...
  filter(predicate: (k: K, v: V) => boolean): HashMap<K, V> {
    return new HashMap<K, V>(
      hamt
        .make({ hash: this.hamt._config.hash, keyEq: this.hamt._config.keyEq })
        .mutate((h: any) => {
          const iterator: Iterator<[K, V]> = this.hamt.entries()
          let curItem = iterator.next()
          while (!curItem.done) {
  ...
  foldLeft<U>(zero: U, fn: (soFar: U, cur: [K, V]) => U): U {
    return this.hamt.fold(
      (acc: U, v: V, k: K & WithEquality) => fn(acc, [k, v]),
      zero
    )
  }
   ...
   */
  toArray(): Array<[K, V]> {
    return this.hamt.fold((acc: [[K, V]], value: V, key: K & WithEquality) => {
      acc.push([key, value])
      return acc
    }, [])
  }
   ...
   *     => {a:1,b:2}
   */
  toObjectDictionary(keyConvert: (k: K) => string = (k) => k.toString()): {
    [index: string]: V
  } {
  ...
  toJsMap<K2 extends number | string | boolean>(
    keyConvert: (k: K) => K2
  ): Map<K2, V> {
    return this.foldLeft(new Map<K2, V>(), (soFar, cur) =>
      soFar.set(keyConvert(cur[0]), cur[1])
    )
  }
  ...
  equals(other: IMap<K & WithEquality, V & WithEquality>): boolean {
    ...
    const keys: Array<K & WithEquality> = Array.from<K & WithEquality>(
      this.hamt.keys()
    )
    for (let k of keys) {
  ...
  hashCode(): number {
    return this.hamt.fold(
      (acc: number, value: V, key: K & WithEquality) =>
        getHashCode(key) + getHashCode(value),
      0
    )
  }
...
export const hashMapOf = HashMap.of
export const hashMapOfIterable = HashMap.ofIterable
export const hashMapOfObject = HashMap.ofObjectDictionary
